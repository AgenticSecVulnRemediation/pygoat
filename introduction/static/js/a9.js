...
event3 = function(){
    ...
    .then(result => {
        ...
        // SECURITY NOTE: All log outputs are safely rendered using textContent; ensure similar patterns elsewhere are updated for XSS prevention
        for (let i = 0; i < data.logs.length; i++) {
            let li = document.createElement("li"); // using let for proper scoping
            li.textContent = data.logs[i]; // XSS remediation: using textContent to safely output log data
